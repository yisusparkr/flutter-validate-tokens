...
class GetValidTokensPage extends StatelessWidget {
          ...
          child: AnimatedContainer(
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  Text('Input your valid tokens', style: GoogleFonts.abel( fontSize: 30.0, color: Colors.white ), ),
  ...
  void validateText( BuildContext context ){
    ...
    try{
      ...
      userWords.forEach((element) { 
        wrongTokens = words[element];
        element.split('').map((character) {
          tokensProvider.tokens.forEach((token) { 
            if( character != token && !wrongTokens.contains(character) ) wrongTokens.add(character);
          });
        }).toList();
        words[element] = wrongTokens;
      });

      userWords.forEach((element) { 
        words[element].removeWhere((character) => tokensProvider.tokens.contains(character));
      });
      ...
      words.forEach((key, values) { 
        if( values.isNotEmpty ){
          ...
          if( !hasInvalidTokens ) hasInvalidTokens = true;
        }
      });
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          title: Center(
            child: Text('Wrong words', style: GoogleFonts.abel( fontSize: 25.0, color: Colors.black87, fontWeight: FontWeight.bold ))
          ),
          content: Container(
            height: 300.0,
            width: 300.0,
            child: ListView.builder(
              ...
              itemBuilder: ( _ , index ){
                return Text('${wrongWords[index]}', style: GoogleFonts.abel( fontSize: 20.0, color: Colors.black87 ));
              },
            ),
          ),
          actions: [
